...
class CrawlerZtLogisticData(BaseCrawler):
    ...
    def get_order_management(self):
        ...
        while True:
            ...
            for row in data_list:
                if not row['fbanumber'] and not row['customernumber1']:  # 同时为空过滤
                    continue
                elif not row['fbanumber']:  # 客户不为空
                    row['fbanumber'] = row['customernumber1']
                row_copy = copy.deepcopy(row)
                if not row_copy.get('receiptdate'):
                    row_copy['dt'] = datetime.now().strftime('%Y%m%d')
                else:
    ...
    def del_zto_data(self):
        sql = 'truncate ods_prod.ods_scg_wld_logistics_zto_i_d'
