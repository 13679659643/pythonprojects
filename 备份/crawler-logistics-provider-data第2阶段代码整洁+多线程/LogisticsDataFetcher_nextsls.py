...
class LogisticsDataFetcher_nextsls(BaseCrawler):
    ...
    def run_producer_consumer_threads(self, shipped_status, total_pages):
        # 主页面数据 生产者
        self.fetch_all_data(shipped_status, total_pages)

        # 创建消费者线程： 列表推导式来创建四个消费者线程,遍历列表，循环启动四个线程.(利用多核 CPU 的有效方式)
        consumer_threads = [threading.Thread(target=self.process_data_from_queue) for _ in range(4)]
        # 等待消费者线程结束:遍历线程,阻塞当前线程,直到所有的消费者线程都结束.
        self.start_and_join_threads(consumer_threads)

        # 插入数据到数据库
        db_insert_thread = [threading.Thread(target=self.insert_data_to_db_from_queue) for _ in range(2)]
        self.start_and_join_threads(db_insert_thread)

    def run_view_threads(self):
        # 创建view页面生产者线程：四个线程去拿每页的id，requests数据。
        viewproducer_threads = [threading.Thread(target=self.fetch_view_data) for _ in range(4)]
        # 等待view生产者线程结束
        self.start_and_join_threads(viewproducer_threads)

        # 处理
        viewconsumer_threads = [threading.Thread(target=self.process_view_from_queue) for _ in range(4)]
        self.start_and_join_threads(viewconsumer_threads)
    ...
    def run(self):
        ...
        for shipped_status in type_list:
            try:
                ...
                self.run_producer_consumer_threads(shipped_status, total_pages)
                logger.info(f'当前主页面数据采集： {self.logistics_provider} {shipped_status} 采集完成.')

                if shipped_status != "cancelled":
                    self.run_view_threads()
                    logger.info(f'当前详情页面数据采集： {self.logistics_provider} {shipped_status} 采集完成.')


            except Exception as e:
                logger.error(f"处理 {shipped_status} 状态时发生错误: {e}")

        logger.info(f'程序.Time: {time.time() - start_time_:.2f}s')

    def main(self):
        self.tidb.commit_sql(truncate_ods_scg_wld_logistics_trace_table_i_d)
        self.tidb.commit_sql(truncate_ods_scg_wld_logistics_trace_view_i_d)
