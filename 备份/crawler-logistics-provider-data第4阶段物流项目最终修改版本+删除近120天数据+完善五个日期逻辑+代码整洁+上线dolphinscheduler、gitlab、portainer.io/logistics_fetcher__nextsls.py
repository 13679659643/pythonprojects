...
class LogisticsDataFetcher_nextsls(BaseCrawler):
    ...
    def __init__(self):
        ...
        # 2024-09-20 09:19:35.030228
        self.now_date = datetime.now()
        # 20240920
        self.end_date = datetime.now().strftime('%Y%m%d')
        # 20240523
        self.start_date = (self.now_date - timedelta(days=120)).strftime('%Y%m%d')
        T = TimestampFormatter()
        self.end_unix = T.format_timestamp(self.end_date)
        self.start_unix = T.format_timestamp(self.start_date)
    ...
    def del_logistics_data(self):
        ...
        :return:
        """
        sql_table = f"delete from ods_prod.ods_scg_wld_logistics_trace_table_i_d WHERE DATE_FORMAT(created,'%Y%m%d')>='{self.start_date}' and DATE_FORMAT(created,'%Y%m%d')<='{self.end_date}'"
        self.tidb.commit_sql(sql_table)
        sql_view = f"delete from ods_prod.ods_scg_wld_logistics_trace_view_i_d WHERE DATE_FORMAT(created,'%Y%m%d')>='{self.start_date}' and DATE_FORMAT(created,'%Y%m%d')<='{self.end_date}'"
        self.tidb.commit_sql(sql_view)
        logger.info("删除东方环球、澳得亚物流、美通物流数据 120天成功")
