...
class CrawlerAhcLogisticData(BaseCrawler):
    ...
    def etl_track_data(self, row_data: list):
        ...
        for index, row in enumerate(reversed_data):
            ...
            if '已开航' in track or '已开船' in track or '预计开船' in track or '开船' in track or '已离境' in track or '已离开起运港' in track:
            ...
            elif 'on its way to our depot' in track:
                ...
                if arrived_at_facility_count == 1:
                    ...
                    track_dict['delivery_date'] = row['zztm']
            elif '已签收' in track or 'Delivered' in track or 'delivered' in track:
                track_dict['signing_date'] = row['zztm']
        return track_dict
