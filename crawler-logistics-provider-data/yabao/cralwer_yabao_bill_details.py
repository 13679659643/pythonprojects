...
# @Time    : 2025/1/8 14:57
# @Author  : Night
# @File    : cralwer_yabao_bill_details.py
# @Description:
import json
...
from db_model import ods_scg_wld_yabao_bill_detail_table, \
...
class CrawlerYaBaoBillDetails(BaseCrawler):
    def __init__(self):
        super().__init__()
    ...
    def get_bill_detail_list(self, page: int = 1):
        """
        :param page: 页数
        :return:
        """
        ...
        response = self.session.get(
            'http://47.106.234.129:808/Home/GetCusBillDetailList',
            params=params,
            verify=False,
        )
        if response.status_code == 200:
    ...
    def fetch_all_pages(self):
        ...
        pageNo = 1
        while True:
            ret_data = self.get_bill_detail_list(pageNo)
            if not ret_data:
                break
    ...
    def del_yb_data(self):
        """
        清空 ahc 近 3个月的数据
        订单起始时间 和终止时间  后台筛选存在问题
        :return:
        """
        sql = f"delete from ods_prod.ods_scg_wld_yabao_bill_detail_i_d WHERE dt>='{self.start_date.replace('-','')}' and dt<='{self.end_date.replace('-','')}'"
        self.tidb.commit_sql(sql)
        logger.info("删除 亚豹账单明细数据 90天成功")
    ...
    def insert_to_tidb(self):
        ...
        :return:
        """
        self.tidb.insert_data(ods_scg_wld_yabao_bill_detail_table,
                              ods_scg_wld_yabao_bill_detail_i_d_field_list,
                              self.data_list)
        logger.info(f'亚豹海外仓-账单明细数据同步{len(self.data_list)}完成')
