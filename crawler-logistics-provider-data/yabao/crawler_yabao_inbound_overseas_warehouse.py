...
# @Time    : 2025/1/8 11:59
# @Author  : Night
# @File    : crawler_yabao_inbound_overseas_warehouse.py
# @Description:
import json
...
from db_model import ods_scg_wld_yabao_inbound_order_list_table, \
    ods_scg_wld_yabao_inbound_order_list_i_d_field_list, ods_scg_wld_yabao_inbound_order_detail_table, \
...
class CrawlerYaBaoInboundOverseasWarehouse(BaseCrawler):
    def __init__(self):
        super().__init__()
    ...
    def get_entry_order_list(self, page: int = 1):
        """
        :param page: 页数
        :return:
        """
        CreateTime = f"{self.start_date} ~ {self.end_date}"
        params = {
            'EntryType': '',
            'page': str(page),
            'limit': '10',
        ...
        response = self.session.get(
            'http://47.106.234.129:808/Home/GetEntryOrderList',
            params=params,
            verify=False,
        )
        if response.status_code == 200:
    ...
    def get_entry_order_detail(self, page: int = 1, eid: int = 0):
        """
        :param page: 页数
        :param eid: 详情id
        :return:
        ...
        response = self.session.get(
            'http://47.106.234.129:808/Home/GetEntryOrderDetail',
            params=params,
            verify=False,
        )
        if response.status_code == 200:
    ...
    def etl_detail_data(self, dataList):
        """
        列表数据
        :param dataList:
        :return:
        ...
        for row in dataList:
            row['dt'] = row['CreateTime'].split(" ")[0].replace("-", "")
            self.data_detail_list.append(row)

    def fetch_detail_all_pages(self, dataList):
        """
        :param dataList: 数据列表
        :return:
        """
        for row in dataList:
            eid = row['EntryOrderID']
            pageNo = 1
            while True:
                ret_data = self.get_entry_order_detail(pageNo, eid)
                if not ret_data:
                    break
                dataList = ret_data['data']
                total = ret_data['count']
                page_total_ct = (total - 1) // 90 + 1
                self.etl_detail_data(dataList)
    ...
    def fetch_all_pages(self):
        ...
        pageNo = 1
        while True:
            ret_data = self.get_entry_order_list(pageNo)
            if not ret_data:
                break
            dataList = ret_data['data']
            total = ret_data['count']
            page_total_ct = (total - 1) // 10 + 1
            self.etl_data(dataList)
            self.fetch_detail_all_pages(dataList)
            if pageNo >= page_total_ct:
                break
            pageNo += 1
    ...
    def insert_to_tidb(self):
        ...
        :return:
        """
        self.tidb.insert_data(ods_scg_wld_yabao_inbound_order_list_table,
                              ods_scg_wld_yabao_inbound_order_list_i_d_field_list,
                              self.data_list)

        self.tidb.insert_data(ods_scg_wld_yabao_inbound_order_detail_table,
                              ods_scg_wld_yabao_inbound_order_detail_i_d_field_list,
                              self.data_detail_list)
        logger.info(f'亚豹海外仓-入库单列表同步{len(self.data_list)}完成')
        logger.info(f'亚豹海外仓-入库单详情同步{len(self.data_detail_list)}完成')
...
if __name__ == '__main__':
    CrawlerYaBaoInboundOverseasWarehouse().main()
