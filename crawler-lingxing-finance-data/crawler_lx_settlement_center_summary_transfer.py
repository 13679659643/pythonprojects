...
class FcSettlementSummary(CrawlerBase):
    ...
    def get_days(self):
        sql = """
                select run_cycle from dim_prod.dim_dsd_me_server_scheduler_i_manual where service_name = 'query_clearing_center_summary_transfer' LIMIT 1;
                """
        days = self.tidb_ob.query_one(sql).get('run_cycle', 31)
        return days

    @Decorate.def_retry(msg="领星财务-亚马逊结算中心-结算汇总(转账) 请求失败！")
    def get_response(self, start_date, end_date, offset=0, length=100, _code=1):
        ...
        json_data = {
            ...
            'sortType': 'desc',
            'req_time_sequence': '/bd/sp/api/settlement/summary/list$$2',
        }
    ...
    def init_task(self):
        ...
        logger.info(f"亚马逊结算中心-结算汇总(转账) 生成任务队列 {len(task_list)} 条！")
        return task_list
    ...
    def init_consumer(self, task_list):
        ...
        logger.info("crawler:领星财务-亚马逊结算中心-结算汇总(转账) 任务消耗完成，程序退出！")
